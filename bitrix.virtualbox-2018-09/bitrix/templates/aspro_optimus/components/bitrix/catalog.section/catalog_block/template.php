...
<?if( count( $arResult["ITEMS"] ) >= 1 ){?>
    ...
    <?php
    //Bitrix\Main\Diag\Debug::writeToFile(array('template' => 'catalog_block',  ),"","log.txt");
    ?>
										...
										<div class="counter_wrapp <?=($arItem["OFFERS"] && $arParams["TYPE_SKU"] == "TYPE_1" ? 'woffers' : '')?>">
											<?if(($arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_LIST"] && $arAddToBasketData["ACTION"] == "ADD") && $arItem["CAN_BUY"]):?>
												<div class="counter_block" data-offers="<?=($arItem["OFFERS"] ? "Y" : "N");?>" data-item="<?=$arItem["ID"];?>">
                                                    <? if(isset($arItem['PROPERTIES']['min_count_4_order']['VALUE'])){
                                                        if ($arAddToBasketData["MIN_QUANTITY_BUY"] < $arItem['PROPERTIES']['min_count_4_order']['VALUE'])
                                                            $arAddToBasketData["MIN_QUANTITY_BUY"] = $arItem['PROPERTIES']['min_count_4_order']['VALUE'];
                                                    };?>
													<span class="minus" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['QUANTITY_DOWN']; ?>" data-min="<?=$arAddToBasketData["MIN_QUANTITY_BUY"];?>">-</span>
