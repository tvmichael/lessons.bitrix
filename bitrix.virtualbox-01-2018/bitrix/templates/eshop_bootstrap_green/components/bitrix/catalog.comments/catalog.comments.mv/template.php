...
if (!$templateData['BLOG']['BLOG_FROM_AJAX'])
{
    ...
    if (!empty($arData))
    {
        ...
        ?>
        <!-- update-01 comments.mv -->
        <div id="<? echo $templateData['TABS_FRAME_ID']; ?>"
             class="bx_soc_comments_div bx_important <? echo $templateData['TEMPLATE_CLASS']; ?>">
            <?
            $content = "";
            $activeTabId = "";
            $tabIDList = array();
            ?>
            <!-- update-02 comments.mv -->
