...
if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true) die();
	...
	$dataTofile = array('1_date'=>date("H:i:s"));
...
$arParams = array(
	...
	'VOTE_LIST_DATA' => $arParams["VOTE_LIST_DATA"],
);
...
if($arParams["ELEMENT_ID"] <= 0)
{
	...
	// LOG-----------------------------------------------------------------------
	$dataTofile = array('3_arParams'=>$arParams);
	Debug::writeToFile($dataTofile,"","/bitrix/tmp/log_mv.log");
	// ---------------------------------------------------------------------------
...
)
{
	...
	// LOG-----------------------------------------------------------------------
	$dataTofile = array('4_1_REQUEST'=>$_REQUEST);
	Debug::writeToFile($dataTofile,"","/bitrix/tmp/log_mv.log");
	// ---------------------------------------------------------------------------

	// LOG-----------------------------------------------------------------------
	$dataTofile = array('4_2_SERVER'=>$_SERVER);
	Debug::writeToFile($dataTofile,"","/bitrix/tmp/log_mv.log");
	// ---------------------------------------------------------------------------

	// LOG-----------------------------------------------------------------------
	$dataTofile = array('4_3_SESSION'=>$_SESSION);
	Debug::writeToFile($dataTofile,"","/bitrix/tmp/log_mv.log");
	// ---------------------------------------------------------------------------
	...
	if( $RATING > 0 && $RATING <= $arParams["MAX_VOTE"] )
	{
		...
		// LOG-----------------------------------------------------------------------
		$dataTofile = array('5'=>'+', 'RATING'=>$RATING, 'arParams[MAX_VOTE]'=>$arParams["MAX_VOTE"],);
		Debug::writeToFile($dataTofile,"","/bitrix/tmp/log_mv.log");
		// ---------------------------------------------------------------------------
		...
		$ELEMENT_ID = intval($_REQUEST["vote_id"]); // код товара за корий голосуем




		$elementDataList = $_REQUEST["vote_list"];
		if(!is_array($elementDataList))
		{
			$elementDataList = array(
				'0'=> array(
					'IBLOCK_ID' => $arParams["IBLOCK_ID"], 			//
					'ELEMENT_ID'=> intval($_REQUEST["vote_id"]),	// код товара за корий голосуем // 
			...
			// LOG-----------------------------------------------------------------------
			$dataTofile = array('5_1_foreeach'=>'+', 'elementDataList'=>$elementDataList);
			Debug::writeToFile($dataTofile,"","/bitrix/tmp/log_mv.log");
			// ---------------------------------------------------------------------------
		foreach ($elementDataList as $valueID) 
		{
			...
			if($ELEMENT_ID > 0 && !array_key_exists($ELEMENT_ID, $_SESSION["IBLOCK_RATING"]))
			{
				...
				$_SESSION["IBLOCK_RATING"][$ELEMENT_ID]=true;	// если за товар проголосовали ставив (true). ELEMENT_ID - код товара
				...
				if($res)
				{
					...
					// LOG-----------------------------------------------------------------------
					$dataTofile = array('7_arParams'=>$arParams);
					Debug::writeToFile($dataTofile,"","/bitrix/tmp/log_mv.log");
					// ---------------------------------------------------------------------------

					// LOG-----------------------------------------------------------------------
					$dataTofile = array('7_arProperties'=>$arProperties);
					Debug::writeToFile($dataTofile,"","/bitrix/tmp/log_mv.log");
					// ---------------------------------------------------------------------------
...
if($this->StartResultCache(false, array($USER->GetGroups(), $bVoted)))
{
	...
	// LOG-----------------------------------------------------------------------
	$dataTofile = array('28_arParams'=>$arParams);
	Debug::writeToFile($dataTofile,"","/bitrix/tmp/log_mv.log");
	// ---------------------------------------------------------------------------
...
if(array_key_exists("AJAX", $arResult) && ($_REQUEST["AJAX_CALL"] != "Y"))
{
	...
	// LOG-----------------------------------------------------------------------
	$dataTofile = array('13_SESSION'=>$_SESSION);
	Debug::writeToFile($dataTofile,"","/bitrix/tmp/log_mv.log");
	// ---------------------------------------------------------------------------
