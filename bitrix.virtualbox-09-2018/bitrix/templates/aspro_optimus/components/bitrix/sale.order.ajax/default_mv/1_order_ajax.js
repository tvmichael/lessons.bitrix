...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function(parameters)
		{
			...
			//
            this.authenticate = parameters.authenticate;
			...
			{
				//console.log(this.totalBlockNode.querySelector('.bx-soa-cart-total'));
                this.totalAuthBlockNode = this.totalBlockNode.querySelector('.bx-soa-auth-block');
		...
		sendRequest: function(action, actionData)
		{
			...
			{
                console.log('  sendRequest: -- saveOrderAjax');
		...
		reachGoal: function(goal, section) // yandex metrika
		{
		...
		getGoalId: function(goal, section) // yandex metrika
		{
			if (!goal)
				return '';
		...
		locationsCompletion: function() // загрузка поля з регіоном домставки
		{
		...
		{
            console.log('>');console.log('clickNextAction:');
			...
			this.reachGoal('next', actionSection); // yandex metrika


			if ( // виконуються коли блок відображається перший раз
				(!this.result.IS_AUTHORIZED || typeof this.result.LAST_ORDER_DATA.FAIL !== 'undefined')
				&& section.next.getAttribute('data-visited') == 'false'
			)
			{
                console.log('clickNextAction: -- if');
		...
		{
            console.log('>');console.log('clickPrevAction:');
		...
		fade: function(node, nextSection)
		{
			...
			switch (node.id)
			{
				...
				case this.basketBlockNode.id:
					...
					break;
				case this.regionBlockNode.id:		// регіон-місто-індекс
		...
		/**
		 * Checks each active block from top to bottom for errors
		 * (showing first block with errors or last block)
		 */
		...
		editOrder: function()
		{
			...
			var sections = this.orderBlockNode.querySelectorAll('.bx-soa-section.bx-active'), i; // вибираємо усі блоки
            //console.log(sections);
			for (i in sections) // перебираємо обєкт з блоками
			{
                console.log('i=:', i);
                //console.log(sections.hasOwnProperty(i));
				if (sections.hasOwnProperty(i)) // якщо елемент масиву існує
				{
					this.editSection(sections[i]); // редагуємо блок
				}
			}
		...
		{
			console.log('editSection: ' + section.id);
			if (!section || !section.id)
				return;
			...
			var active = section.id == this.activeSectionId,	// перевіряємо чи поточна секція активна
				titleNode = section.querySelector('.bx-soa-section-title-container'), // заголовок поточного блоку
			...
			{
				BX.bind(titleNode, 'click', BX.proxy(this.showByClick, this));	// навішуємо підої на заголовок поточного блоку
				editButton = titleNode.querySelector('.bx-soa-editstep');
				editButton && BX.bind(editButton, 'click', BX.proxy(this.showByClick, this)); // + аналогічно на посилання
			}
			...
			{
				case this.authBlockNode.id:
					this.editAuthBlock();			// блок авторизації користувача - не появляється якщо стоїть галочка "Оформлять заказ с автоматической регистрацией пользователя:"
					break;
				case this.basketBlockNode.id:
					this.editBasketBlock(active);	// блок з зомовленими товарами
					break;
				case this.regionBlockNode.id:
					this.editRegionBlock(active);	// регіон доставки - місто-індекс
					break;
				case this.paySystemBlockNode.id:	// системи оплати
					this.editPaySystemBlock(active);
					break;
				case this.deliveryBlockNode.id:		// служби доставки
					this.editDeliveryBlock(active);
					break;
				case this.pickUpBlockNode.id:		// ?
					this.editPickUpBlock(active);
					break;
				case this.propsBlockNode.id:		// властивсті користувача імя-емаіл-телефон-адрес...
		...
		editBasketBlock: function(active) // блок з зомовленими товарами
		{
		...
		editFadeRegionBlock: function() // регіон-місто-нндекс...
		{
		...
		editPropsBlock: function(active)	// редагє властивсті користувача: імя-емаіл-телефон-адрес...
		{
			console.log('editPropsBlock:');
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			//console.log('getPropertyRowNode');
		...
		editTotalBlock: function() // правий блок загальної суми
		{
